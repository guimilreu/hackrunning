...
const NAV_CATEGORIES = [
  ...
  },
  {
    label: 'Treinamento',
    items: [
      { label: 'Treinos', href: '/app/training-plan', icon: Calendar },
    //   { label: 'Aulas', href: '/app/classes', icon: BookOpen },
    ]
  },
  {
    label: 'Gamificação',
  ...
  },
  {
    ...
    items: [
      { label: 'Eventos', href: '/app/races', icon: Award },
      { label: 'Loja', href: '/app/store', icon: ShoppingBag },
    ]
  },
];
...
export default function AppLayout({ children }) {
  ...
  const { data: onboardingData, isLoading: loadingOnboarding } = useOnboarding();
  ...
  const isOnboardingRoute = pathname.includes('/app/onboarding');
  const isCheckoutRoute = pathname.includes('/app/checkout');
  const isPaymentPendingRoute = pathname.includes('/app/onboarding/payment-pending');
  ...
  useEffect(() => {
    ...
    if (_hasHydrated && isAuthenticated && !isOnboardingRoute && !isCheckoutRoute && !loadingOnboarding && !hasPaymentPending) {
      if (!onboardingComplete) {
        ...
        const stepPaths = {
          1: '/app/onboarding/step1',
          2: '/app/onboarding/step2',
          3: '/app/onboarding/step3',
          4: '/app/onboarding/step4',
          5: '/app/onboarding/step5',
          6: '/app/onboarding/step6',
          7: '/app/onboarding/kickstart-kit'
        };
        router.push(stepPaths[currentStep] || '/app/onboarding/step1');
      }
    }
  ...
  const NavContent = () => (
    <div className="flex flex-col h-full">
        <div className="mb-6 px-2">
           <Link href="/app/home" className="flex items-center group">
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto custom-scrollbar px-2">
          ...
          {/* Meu Perfil */}
          <Link href="/app/profile" className="mt-2">
            <div
              className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group ${
            ...
            >
              <User className={`h-5 w-5 ${pathname.startsWith('/app/profile') ? "text-primary" : "group-hover:text-primary transition-colors"}`} />
              <span className={`font-medium ${pathname.startsWith('/app/profile') ? "text-primary" : ""}`}>Meu Perfil</span>
            </div>
          </Link>
        </nav>
        ...
        <div className="mt-auto pt-4 border-t border-white/5 px-2 space-y-2">
          <Link href="/app/hpoints" className="group">
